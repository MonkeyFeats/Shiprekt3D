...
class Frustum
{
	...
	void Update(const float[]&in proj_view)
	{
		...
		plane2.distance = proj_view[15] + proj_view[12];
	...
	{
		if (!plane0.IntersectsAABB(box))
			return false;
		if (!plane1.IntersectsAABB(box))
			return false;
		if (!plane2.IntersectsAABB(box))
			return false;
		if (!plane3.IntersectsAABB(box))
			return false;
		if (!plane4.IntersectsAABB(box))
			return false;
		if (!plane5.IntersectsAABB(box))
			return false;
		return true;
	}
